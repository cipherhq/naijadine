...
INSERT INTO public.notification_templates (name, channel, subject, body, whatsapp_template_name) VALUES
  ...
  ('welcome_wa', 'whatsapp', NULL, 'Welcome to NaijaDine! Discover 500+ restaurants in Lagos, Abuja & PH. Start: {{link}}', 'welcome_message'),
  ('welcome_email', 'email', 'Welcome to NaijaDine 🍽️', 'Hi {{first_name}}, welcome to NaijaDine! Your ₦1,000 welcome credit is ready. Explore restaurants: {{link}}', NULL),
  ...
  ('payout_confirmation_email', 'email', 'Payout Processed — NaijaDine', 'Your payout of {{amount}} has been sent to {{bank}} ****{{last4}}. Transfer ref: {{ref}}.', NULL),
  ('deal_alert_wa', 'whatsapp', NULL, '{{restaurant}} has {{discount}}% off this {{period}}! Book now: {{link}}', 'deal_alert'),
  ('restaurant_approved_email', 'email', 'Welcome to NaijaDine — You''re Approved!', 'Congratulations! {{restaurant}} is now live on NaijaDine. Log in to your dashboard: {{link}}', NULL),
